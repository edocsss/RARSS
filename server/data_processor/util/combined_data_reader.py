...
def _read_smartphone_combined_data(activity_type):
    file_path = os.path.join(CONFIG.COMBINED_DATA_DIR, activity_type, CONFIG.FILE_NAME_SUFFIX + CONFIG.PREPROCESS_DATA_SOURCE_SUBJECT + '_' + CONFIG.COMBINED_DATA_RESULT['sp'])
    return _read_csv_data(file_path)


def _read_smartwatch_combined_data(activity_type):
    file_path = os.path.join(CONFIG.COMBINED_DATA_DIR, activity_type, CONFIG.FILE_NAME_SUFFIX + CONFIG.PREPROCESS_DATA_SOURCE_SUBJECT + '_' + CONFIG.COMBINED_DATA_RESULT['sw'])
    return _read_csv_data(file_path)
